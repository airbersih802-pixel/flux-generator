...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.error("🔒 Aplikasi ini terkunci. Silakan beli E-Book Panduan untuk dapat kodenya.")
        return False
        ...
        st.error("❌ Kode Salah! Coba cek lagi di E-Book halaman terakhir.")
        return False
    else:
        # Password correct.
        return True
...
if check_password():
    # --- JUDUL APLIKASI ---
    st.title("📸 FLUX.2 Hyper-Realism Generator")
    ...
    # --- TOMBOL EKSEKUSI ---
    st.write("")
    if st.button("✨ GENERATE MANTRA AJAIB ✨"):
        if objek:
            st.success("✅ Prompt Berhasil Dibuat! Silakan Copy:")
            ...
            st.info("💡 **Tips:** Copy prompt ini ke **Tensor.art** atau **SeaArt** pakai model **FLUX.2**.")
        else:
            st.warning("⚠️ Eits, isi dulu deskripsi objeknya bro!")
